...
const submitPayment = async (req, res) => {
  try {
    const { bookingId, renterId, ownerId, amount, paymentMethod, details } =
      req.body;
...
const getOwnerTransactions = async (req, res) => {
  try {
    ...
    const txns = await Transaction.find({ ownerId }).populate(
      "renterId",
      "name email"
    );
...
const getRenterTransactions = async (req, res) => {
  try {
    ...
    const txns = await Transaction.find({ renterId }).populate(
      "ownerId",
      "name email"
    );
    res.status(200).json(txns);
